...
import aiohttp
import discord
from redbot.core import Config, bank, checks, commands, data_manager
...
class Obfuscator(commands.Cog):
    ...
    async def obfuscate(
        self, ctx: commands.Context, type: Optional[commands.Context] = None
    ) -> None:
        """
        Lua Obfuscator
    ...
    async def _luaseel(self, ctx: commands.Context):
        ...
        """
        if (
            await self.config.user(ctx.author).is_whitelisted()
            and ctx.message.channel.type is discord.ChannelType.private
        ):
            letters = string.ascii_uppercase
            filename = "".join(random.choice(letters) for i in range(7))
            x = re.findall(r"(?<=```)[\S\s]*(?=```)", ctx.message.content)
            obfuscated = "{}/{}/{}".format(
                file_path, "obfuscated", filename + "-obfuscated.lua"
            )
            ...
            if x:
                ...
                if not os.path.exists(obfuscated):
                    ...
                    embed = discord.Embed(
                        title="Error",
                        description="\nVerify your syntax is correct and try again.",
                        color=0xED4245,
                    )
                ...
                embed = discord.Embed(
                    ...
                    description=("\nFile obfuscated in ⌛({}) seconds.").format(
                        str(time_elapsed)[:5]
                    ),
                    color=0x000088,
                )
                ...
                if not ctx.message.attachments[0].url.endswith((".lua", ".txt")):
                    ...
                    embed = discord.Embed(
                        ...
                        description=f"only ``.lua`` or ``.txt`` allowed",
                        color=0xED4245,
                    )
                    return await ctx.send(embed=embed)
            ...
            if not await bank.can_spend(ctx.author, cost):
                await ctx.send(
                    ("You don't have enough {} ({}). Obfuscator cost: {} {}.").format(
                        currency, balance, cost, currency
                    )
                )
                return
            if (
                not ctx.message.author.bot
                and ctx.message.channel.id
                in await self.config.guild(ctx.guild).channels()
            ):
                letters = string.ascii_uppercase
                filename = "".join(random.choice(letters) for i in range(7))
                x = re.findall(r"(?<=```)[\S\s]*(?=```)", ctx.message.content)
                ...
                try:
                    if not os.path.exists(node_folder):
                        subprocess.check_output(
                            f"cd {file_path} && npm install",
                            shell=True,
                            stderr=subprocess.STDOUT,
                        )
                        subprocess.check_output(
                            f"cd {file_path} && node menprotect_cli.js",
                            shell=True,
                            stderr=subprocess.STDOUT,
                        )
                    subprocess.check_output(
    ...
    async def _prometheus(self, ctx: commands.Context):
        ...
        if (
            ...
            if x:
                ...
                try:
                    subprocess.check_output(
                        f"cd {file_path} && lua prometheus_cli.lua --preset Medium {path}",
                        shell=True,
                        stderr=subprocess.STDOUT,
                    )
                except subprocess.CalledProcessError as err:
    ...
    async def _ironbrew(self, ctx: commands.Context):
        ...
        if (
            ...
            if x:
                ...
                try:
                    subprocess.check_output(
                        f"cd {file_path} && dotnet IronBrew2CLI.dll {path}",
                        shell=True,
                        stderr=subprocess.STDOUT,
                    )
                except subprocess.CalledProcessError as err:
    ...
    async def _bytecode(self, ctx: commands.Context):
        ...
        if (
            ...
            if x:
                ...
                try:
                    subprocess.check_output(
                        f"cd {file_path} && lua bytecode_cli.lua --cli --source {path} --output {obfuscated} --comment {ctx.message.author.display_name} --varcomment {ctx.message.author.display_name} --cryptvarcomm True --varname {ctx.message.author.display_name}",
                        shell=True,
                        stderr=subprocess.STDOUT,
                    )
                except subprocess.CalledProcessError as err:
    ...
    async def _xor(self, ctx: commands.Context):
        ...
        if (
            ...
            x = re.findall(r"(?<=```)[\S\s]*(?=```)", ctx.message.content)
            obfuscated = "{}/{}/{}".format(
                file_path, "obfuscated", filename + "-unpacked.lua"
            )
            upload = "{}/{}/{}".format(file_path, "uploads", filename + ".lua")
            start_time = time.time()
            ...
            if x:
                ...
                try:
                    subprocess.check_output(
                        f"cd {file_path} && dotnet XOR.dll {path}",
                        shell=True,
                        stderr=subprocess.STDOUT,
                    )
                except subprocess.CalledProcessError as err:
                ...
                embed = discord.Embed(
                    title="📂 File unpacked",
                    description=("\nFile unpacked in ⌛({}) seconds.").format(
                        str(time_elapsed)[:5]
                    ),
                    color=0x000088,
                )
    ...
    async def _minify(self, ctx: commands.Context):
        ...
        if (
            ...
            x = re.findall(r"(?<=```)[\S\s]*(?=```)", ctx.message.content)
            obfuscated = "{}/{}/{}".format(
                file_path, "obfuscated", filename + "-minified.lua"
            )
            upload = "{}/{}/{}".format(file_path, "uploads", filename + ".lua")
            ...
            if x:
                ...
                try:
                    output = subprocess.getoutput(
                        f"lua {minify} minify {path} > {obfuscated}"
                    )
                except subprocess.CalledProcessError as err:
                    log.error(f"{err}")
                if not os.path.exists(obfuscated):
                ...
                embed = discord.Embed(
                    title="📂 File minified",
                    description=("\nFile minified in ⌛({}) seconds.").format(
                        str(time_elapsed)[:5]
                    ),
                    color=0x000088,
                )
    ...
    async def _beautify(self, ctx: commands.Context):
        ...
        if (
            ...
            x = re.findall(r"(?<=```)[\S\s]*(?=```)", ctx.message.content)
            obfuscated = "{}/{}/{}".format(
                file_path, "obfuscated", filename + "-beautified.lua"
            )
            upload = "{}/{}/{}".format(file_path, "uploads", filename + ".lua")
            minify = "{}/{}".format(file_path, "minify.lua")
            start_time = time.time()
            ...
            if x:
                ...
                try:
                    output = subprocess.getoutput(
                        f"lua {minify} beautify {path} > {obfuscated}"
                    )
                except subprocess.CalledProcessError as err:
                    log.error(f"{err}")
                if not os.path.exists(obfuscated):
                ...
                time_elapsed = end_time - start_time
                embed = discord.Embed(
                    title="📂 File beautified",
                    description=("\nFile beautified in ⌛({}) seconds.").format(
                        str(time_elapsed)[:5]
                    ),
                    color=0x000088,
                )
    ...
    async def obfuscation_channel(
        ...
        if channel.id in await self.config.guild(ctx.guild).channels():
            ...
            await ctx.send(
                ("{} removed from obfuscator channels.").format(channel.mention)
            )
        else:
            async with self.config.guild(ctx.guild).channels() as data:
    ...
    async def obfuscation_cost(
        self, ctx: commands.context, *, amount: Optional[int] = 0
    ) -> 0:
    ...
    async def obfuscation_user(self, ctx: commands.Context, *, user: discord.Member):
        ...
        if (
            await self.config.user(user).is_whitelisted()
            or user in await self.config.guild(ctx.guild).users()
        ):
            ...
            await ctx.send(
                ("{} removed from obfuscator whitelist.").format(user.mention)
            )
        else:
            async with self.config.guild(ctx.guild).users() as data:
                data.append(user.id)
    ...
    async def obfuscation_version(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            title="Version/About",
            description=(
                "\nObfuscator version: {}\nCog author: [{}](https://discordapp.com/users/309536563161989120)"
            ).format(self.__version__, self.__author__),
            color=0x000088,
        )
        embed.set_thumbnail(
            url="https://github.com/jmesfo0/jmes-cogs/raw/main/obfuscator/lua.png"
        )
        await ctx.send(embed=embed)
    ...
    async def obfuscation_help(self, ctx: commands.Context):
        ...
        embed.add_field(
            name="obfuscate", value="the default obfuscator command", inline=False
        )
        embed.add_field(
            name="obfuscate luaseel", value="LuaSeel Obfuscator (default)", inline=False
        )
        embed.add_field(
            name="obfuscate menprotect", value="Menprotect Obfuscator", inline=False
        )
        embed.add_field(
            name="obfuscate prometheus", value="Prometheus Obfuscator", inline=False
        )
        embed.add_field(
            name="obfuscate ironbrew", value="IronBrew Obfuscator", inline=False
        )
        embed.add_field(
            name="obfuscate bytecode", value="Bytecode Obfuscator", inline=False
        )
        ...
        embed.set_footer(
            text=("Obfuscator ({})").format(self.__version__),
            icon_url="https://github.com/jmesfo0/jmes-cogs/raw/main/obfuscator/lua.png",
        )
        await ctx.send(embed=embed)
    ...
    async def _stats(
        ...
        if not user:
            ...
            embed = discord.Embed(
                title=("📂 Obfuscator Stats for {}").format(ctx.author.display_name),
                colour=0x000088,
            )
            ...
            embed = discord.Embed(
                title=("📂 Obfuscator Stats for {}").format(user.display_name),
                colour=0x000088,
            )
    ...
    async def obfuscatorset_settings(self, ctx: commands.Context):
        ...
        global_watermark = data["watermark"]
        ...
        if global_watermark:
            embed.add_field(
                name="Global Watermark:", value=str(global_watermark), inline=False
            )
        embed.add_field(
            name="Cost:", value=("{} {}").format(str(cost), currency), inline=False
        )
