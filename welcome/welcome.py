...
default_settings = {
    ...
    "FUN_IMAGES": [
        ...
        "https://media.tenor.com/Fg7Jrii-17kAAAAd/james-franco-wait-what.gif",
        "https://media.tenor.com/_hLOQx93h_QAAAAC/wtf-gif.gif",
        "https://media.tenor.com/TQNN6u6MWg0AAAAC/uncomfortable-wtf.gif"
    ],
    "JOINED_TODAY": False,
    "MINIMUM_DAYS": 0,
...
class Welcome(Events, commands.Cog):
    ...
    @welcomeset.group(name="fun", aliases=["funny"])
    async def welcomeset_fun(self, ctx: commands.Context) -> None:
        """
        Manage welcome fun images.
        """
        pass
        
    @welcomeset_fun.command(name="add")
    async def welcomeset_fun_add(
        self, ctx: commands.Context, *, format_msg: str
    ) -> None:
        """
        Adds a funny image to the guild.
        """
        guild = ctx.message.guild
        guild_settings = await self.config.guild(guild).FUN_IMAGES()
        guild_settings.append(format_msg)
        await self.config.guild(guild).FUN_IMAGES.set(guild_settings)
        await ctx.send(_("Fun image added for the guild."))

    @welcomeset_fun.command(name="del")
    async def welcomeset_fun_del(self, ctx: commands.Context) -> None:
        """Removes a funny image from the funny images list."""
        guild = ctx.message.guild
        guild_settings = await self.config.guild(guild).FUN_IMAGES()
        msg = _("Choose a image to delete:\n\n")
        ...
        if not guild_settings:
            guild_settings = []
        await self.config.guild(guild).FUN_IMAGES.set(guild_settings)
        await ctx.send(_("**This image was deleted:**\n") + str(choice))

    @welcomeset_fun.command(name="list")
    async def welcomeset_fun_list(self, ctx: commands.Context) -> None:
        """
        Lists the funny images of this guild
        """
        guild = ctx.message.guild
        msg = _("Funny Images:\n\n")
        guild_settings = await self.config.guild(guild).FUN_IMAGES()
    ...
    @welcomeset_fun.command(name="toggle")
    async def welcomeset_fun_toggle(self, ctx: commands.Context) -> None:
        """
        Turns on/off fun images when users join guild.
        """
        guild = ctx.message.guild
        guild_settings = await self.config.guild(guild).FUN()
        guild_settings = not guild_settings
        if guild_settings:
            await ctx.send(_("I will now send random fun images when users join."))
        else:
            await ctx.send(_("I will no longer send random fun images when users join."))
        await self.config.guild(guild).FUN.set(guild_settings)
